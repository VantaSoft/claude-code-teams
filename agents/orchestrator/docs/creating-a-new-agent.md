...
2. **Scaffold the directory** — run:
   ```bash
   ~/agents/orchestrator/scripts/create-agent.sh <name>
   ```
   This creates the standard folder structure (CLAUDE.md, tasks.md, memory/, docs/, .claude/).

3. **Customize the agent's CLAUDE.md** — Write a role-specific CLAUDE.md in `~/agents/<name>/CLAUDE.md` based on the scope. Include:
...
6. **Launch the agent**:
   ```bash
   ~/agents/orchestrator/scripts/start-agent.sh <name>
   ```

7. **Update the Active Agents table** in `~/CLAUDE.md` with the new agent's name, role, directory, and tmux session.
