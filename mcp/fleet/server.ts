 ...
 *   list_routines(agent?)         — list cron routines across the fleet
 ...
 *   sync_routines()               — re-install MANAGED crontab block
 ...
 * agent_status, list_mcps, list_routines, and sync_routines are
...
// --- routines: load, list, sync to crontab ---------------------------

// Walk every agent's routines/ directory, parse the cron frontmatter
// out of each .md, and return a flat list. Pure-TS reader; shared by
// list_routines and sync_routines so the parser only lives in one
// place.
type Routine = { agent: string; name: string; file: string; cron: string };

function loadRoutines(agent?: string): Routine[] {
  const agents = agent ? [agent] : listAgents();
  const rows: Routine[] = [];
  for (const a of agents) {
    const dir = join(AGENTS_DIR, a, "routines");
    let files: string[];
    try {
...
function listRoutines(agent?: string): string {
  const rows = loadRoutines(agent);
  if (rows.length === 0) return "(no routines found)";
...
// routine .md files. Preserves any non-managed crontab entries
// (between the markers, exclusive on both sides). Atomically rewrites
// via `crontab -` reading from stdin.
const CRON_BEGIN = "# BEGIN MANAGED: agent-routines";
const CRON_END = "# END MANAGED: agent-routines";
...
function syncRoutines(): string {
  const rows = loadRoutines();
  const blockLines: string[] = [
    CRON_BEGIN,
    "# Generated by fleet:sync_routines — do not edit by hand",
    "# Edit routine files in PROJECT_ROOT/agents/<agent>/routines/*.md and re-run the tool",
  ];
  for (const r of rows) {
  ...
  return `Synced ${rows.length} routine(s) to crontab`;
}
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: "sync_routines",
      description:
        "Re-install the MANAGED crontab block by scanning PROJECT_ROOT/agents/*/routines/*.md for cron frontmatter and regenerating the managed block in the user crontab. Preserves any non-managed entries. Safe to run repeatedly. Orchestrator-only.",
      inputSchema: { type: "object", properties: {} },
    },
    {
      name: "list_routines",
      description:
        "List every recurring routine across the fleet by reading PROJECT_ROOT/agents/*/routines/*.md and parsing the cron frontmatter. Returns a table of agent, routine name, and cron expression. Pure file read — does not consult crontab. Pass an agent name to filter to one agent; omit for the whole fleet.",
      inputSchema: {
        type: "object",
        properties: {
          agent: { type: "string", description: "Optional agent name. Omit to list routines for all agents." },
        },
      },
    },
    {
      name: "create_agent",
      description:
        "Scaffold a new agent folder at PROJECT_ROOT/agents/<agent>/ with CLAUDE.md, memory/, docs/, routines/, and .claude/. Fails if the directory already exists. Does NOT wire up Slack or start the agent — run PROJECT_ROOT/agents/orchestrator/scripts/setup-slack.sh manually (it's kept as a standalone script because it takes Slack tokens) and then call restart_agent after.",
      inputSchema: {
        type: "object",
        properties: {
...
mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "sync_routines") {
      try {
        return { content: [{ type: "text", text: syncRoutines() }] };
      } catch (e) {
    ...
    if (name === "list_routines") {
      ...
      if (agent && !isValidAgentName(agent))
        return { content: [{ type: "text", text: `invalid agent name: ${agent}` }], isError: true };
      return { content: [{ type: "text", text: listRoutines(agent) }] };
    }
